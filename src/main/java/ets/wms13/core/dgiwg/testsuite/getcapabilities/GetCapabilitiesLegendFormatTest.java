...
public class GetCapabilitiesLegendFormatTest extends AbstractBaseGetCapabilitiesFixture {
    ...
    public  void wmsCapabilitiesLayersLegendExistsInCorrectFormat( ITestContext testContext, Node layerNode, String name, String title )
                                    throws XPathFactoryConfigurationException, XPathExpressionException //, SOAPException 
    {
        //skipIfNoVectorData( testContext );
        Node formatNode = (Node) createXPath().evaluate( "ancestor-or-self::wms:Layer/wms:Style/wms:LegendURL/wms:Format", layerNode, XPathConstants.NODE );
        assertNotNull( formatNode, "Style/LegendURL/Format element for layer (name: " + name + ", title: " + title + ") is missing." );
